...
- **Header "More" Options Menu**: Decluttered the terminal header by consolidating secondary actions into a compact, responsive "More" (`⋮`) dropdown menu (Persian Language & Input Helper, Search in output, Theme picker, Clear screen, Font size `A−`/`A+`, and Fullscreen toggle). Mobile header now cleanly displays only essential controls without wrapping or crowding.
- **Dual-Row Keyboard Toolbar & Terminal Key Combinations**: Upgraded the accessory keyboard toolbar by removing redundant symbol keys and introducing direct one-tap terminal key combinations (`Ctrl+C`, `Ctrl+D`, `Ctrl+Z`, `Ctrl+L`, `Ctrl+R`, `Ctrl+A`, `Ctrl+E`, `Ctrl+W`, `PgUp`, `PgDn`) with visual feedback and critical color accenting. Preserved toggleable dual-row mode (`2-Row ▾` / `1-Row ▴`) with persistent user preference.
- **Termius-Style File Upload (📎)**: Added one-tap file upload button to the toolbar and "More" dropdown. Users can stream any file from their local device directly to the remote host (`/tmp/upload_<timestamp>_<name>`) over SFTP, with upload progress toast notifications and immediate shell prompt auto-typing of the resulting remote path.
- **Compact Snippet Action Icons**: Streamlined the Quick Commands & Snippets modal by replacing wide text buttons with clean, modern SVG icon buttons (`Run`, `Insert`, `Copy`, `Delete`), allowing full snippet titles and command strings to display prominently without truncation.
- **Persian Typing & Input Protection**: Guarded real-time streaming output against single-character typing echo mangling, preserving standard UTF-8 interactive terminal input while providing full visual shaping and preview via the dedicated Persian Helper modal.
- **Horizontal Tab Swipe Sensitivity & Exclusion**: Fixed accidental tab switching during horizontal scrolling (e.g. Analytics tables and charts). Enforced a significantly higher swipe distance threshold (`Math.max(160, Math.round(window.innerWidth * 0.40))`), horizontal dominance ratio (`|dx| >= |dy| * 2.5`), vertical deflection bounds (`|dy| <= 60px`), gesture duration constraint (`<= 800ms`), and comprehensive element exclusions (`isHorizontalScrollableTarget` checks for scrollable containers, charts, tables, inputs, pre/code blocks, and `.terminal-toolbar`).
- **Tab Switcher Button Typography**: Fixed font inheritance for "New tab" and "Close All" buttons (`.tab-switcher button`, `.tab-switcher__new`, `.tab-switcher__close-all`), ensuring consistent `Exo 2` / `Vazirmatn` font rendering across all platforms.
- **Accessory Keyboard Toolbar On Desktop & Mobile**: Enabled accessory virtual toolbar on desktop with dedicated toggle tool (`#term-tool-keyboard`), persisting user preference and recalculating PTY dimensions automatically.
- **Collapsible Overview & Diagnostics Cards**: Converted dense informational sections into collapsible `<details class="card card--collapsible">` accordions defaulting to closed state (Diagnostics: Recent job runs, Export backup, Restore backup; Overview: Runtime, Collection scheduler, Route groups) with animated disclosure chevron.
- **High-Speed Paste Acceleration**: Accelerated clipboard paste speed by normalizing CRLF to CR for raw PTY, chunking oversized payloads, adding native container and textarea paste listeners, expanding backend input frame capacity from 16KB to 64KB with chunked streaming in `handlers.go`, and burst RAF output buffering.
