...
# The bare `golang:1.26` tag is the minor-tracking tag: it floats to the latest
# 1.26.x patch published on Docker Hub. Deliberately not pinned to an exact
# patch (e.g. golang:1.26.1) so builds pick up Go security/patch releases
# automatically. go.mod's `go 1.26.8` line is the minimum the code requires.
